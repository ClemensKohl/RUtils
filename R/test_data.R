...
preprocess_data_set <- function(
  ...
  SummarizedExperiment::colData(sce) <- cbind(
  ...
  if (!is.null(truth)) {
    plots[["p_counts"]] <- scater::plotColData(
      sce,
      x = truth,
      y = "sum",
      colour_by = "discard"
    ...
    plots[["p_detected"]] <- scater::plotColData(
      sce,
      x = truth,
      y = "detected",
      colour_by = "discard"
    ...
    plots[["p_mito"]] <- scater::plotColData(
      sce,
      x = truth,
  ...
  plots[["p_sumVSdetected"]] <- scater::plotColData(
    sce,
    x = "sum",
    y = "detected",
    colour_by = "discard"
  ...
  plots[["p_hist_sum"]] <- ggplot2::ggplot(
    as.data.frame(SummarizedExperiment::colData(sce)),
    ggplot2::aes(x = sum)
  ) +
    ggplot2::geom_histogram(fill = "#008080") +
  ...
  if (isTRUE(rmbatch)) {
    ...
    cnt_bc <- sva::ComBat(
      counts(sce),
      batch = SummarizedExperiment::colData(sce)[, batchcol]
    )
    if (min(cnt_bc) < 0) {
  ...
  plots[["p_umap_dblscore"]] <- scater::plotUMAP(
    sce,
    colour_by = "scDblFinder.score"
  )

  plots[["p_umap_dblclass"]] <- scater::plotUMAP(
    sce,
