...
plot_hm <- function(
  ...
  breaks <- c(
    min(-1, quantile(mat, 0.01)),
    0,
    2,
    4,
    max(6, quantile(mat, 0.99))
  )
...
make_bic_hm <- function(
  ...
  col_fun <- colorRamp2(
    seq(quantile(cnts, 0.01), quantile(cnts, 0.99), length = 10),
    viridis(10)
  )
...
plot_flow <- function(df, rm_redund = TRUE) {
  ...
  sank <- ggsankey::make_long(df, colnames(df))
  p <- ggplot2::ggplot(
    sank,
    ggplot2::aes(
      x = x,
